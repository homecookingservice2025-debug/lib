...
// In Vercel, we should use /tmp for SQLite if we want to write, 
// but it's ephemeral. For a real app, use a real DB.
const dbPath = process.env.DATABASE_PATH || "/tmp/library.db";
...
app.get("/api/subscriptions/ledger", (req, res) => {
  const ledger = db.prepare(`
    SELECT sub.*, s.name as student_name
...
app.post("/api/attendance/check-in", (req, res) => {
  ...
  if (!seat) return res.status(400).json({ error: "No seats available" });
...
app.post("/api/attendance/check-out", (req, res) => {
  const { student_id } = req.body;
  const active = db.prepare("SELECT * FROM attendance WHERE student_id = ? AND check_out IS NULL").get(student_id);
  if (!active) return res.status(400).json({ error: "Not checked in" });
...
app.get("/api/reports/attendance", (req, res) => {
  const logs = db.prepare(`
    SELECT a.*, s.name as student_name, z.name as zone_name, st.section, st.seat_number
    FROM attendance a
